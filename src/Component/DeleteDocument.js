...
export default function DeletedDocuments({ deletedDocs = [], onRestore = () => {}, onPermanentDelete = () => {} }) {
  return (
    <div className="app">
                ...
                <tbody>
                  {deletedDocs.map((doc, idx) => (
                    <tr key={doc.id}>
                      ...
                      <td><span className={daysLeft(doc.deletedAt) <= 2 ? "warning" : ""}>{daysLeft(doc.deletedAt)} hari</span></td>
                      <td>
                        <div className="action-buttons">
                          <button className="restore-btn" onClick={() => onRestore(doc.id)}>Pulihkan</button>
                          <button className="permanent-delete-btn" onClick={() => { if(window.confirm('Hapus permanen dokumen ini?')) onPermanentDelete(doc.id); }}>Hapus Permanen</button>
                        </div>
                      </td>
                    </tr>
                  ))}
